...
test('waterfall overlapping events', () => {
  const events: CalendarEvent[] = [
    { start: 30, end: 90 },
    { start: 60, end: 120 },
    { start: 95, end: 155 },
  ...
  expect(orderedRes[2].column).toBe(0);
  expect(orderedRes[3].width).toBe(300);
  expect(orderedRes[3].column).toBe(1);
  expect(orderedRes[4].width).toBe(300);
  expect(orderedRes[4].column).toBe(0);
});

test('return valid result when three events overlap', () => {
  const threeOverlappingEvents: CalendarEvent[] = [
    { start: 540, end: 600 },
    { start: 560, end: 620 },
    { start: 580, end: 650 },
  ];
  const res = splitEventsInColumns(threeOverlappingEvents);
...
test('return valid result in complex use case when four events overlap and should be displayed in 3 columns', () => {
  ...
  const res = splitEventsInColumns(fourOverlappingEventsInThreeColumns);
